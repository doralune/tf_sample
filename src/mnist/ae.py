...
def init():
    ...
    # inout
    parser.add_argument("--out_dir", default='out/sample/mnist/ae/dummy2')
...
class Xnn(object):
    ...
    def build_graph(self):
        ...
        with tf.variable_scope('fc1'):
            ...
            w = self.new_weight([data_dim, 1000], reg_coeff=reg_coeff)
            b = self.new_bias([1000], 0, reg_coeff=reg_coeff) # 1
            fc = tf.matmul(data, w)
            bias = fc + b
            fc1 = tf.nn.sigmoid(bias)
            fc1 = tf.nn.dropout(fc1, keep_prob)
            parameters += [w, b]
            print_tensor(fc1)
        with tf.variable_scope('fc2'):
            fc1_dim = fc1.get_shape().as_list()[1]
            w = self.new_weight([fc1_dim, data_dim], reg_coeff=reg_coeff)
            b = self.new_bias([data_dim], 0, reg_coeff=reg_coeff) # 1
            fc = tf.matmul(fc1, w)
            bias = fc + b
            fc2 = bias
            parameters += [w, b]
        ...
        ce = tf.nn.sigmoid_cross_entropy_with_logits(fc2, data, name='ce')
        self.ce_loss = ce_loss = tf.reduce_mean(ce, name='ce_loss')

        self.recon = recon = tf.nn.sigmoid(fc2)
