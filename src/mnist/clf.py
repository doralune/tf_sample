...
def init():
    ...
    # inout
    parser.add_argument("--out_dir", default='out/sample/mnist/clf/dummy2')
    ...
    parser.add_argument("--out_steps", type=int, default=1000)
...
class Xnn(object):
    ...
    def build_graph(self):
        ...
        with tf.variable_scope('fc1'):
            ...
            b = self.new_bias([100], 0, reg_coeff=reg_coeff) # 1
            fc = tf.matmul(data, w)
            bias = fc + b
            fc1 = tf.nn.relu(bias)
        ...
        with tf.variable_scope('logits'):
            ...
            w = self.new_weight([fc1_dim, cls_num], reg_coeff=reg_coeff)
            b = self.new_bias([cls_num], 0, reg_coeff=reg_coeff) # -7
        ...
        self.correct_num = correct_num = tf.reduce_sum(tf.cast(correct, tf.int32))
        accuracy = tf.reduce_mean(tf.cast(correct, tf.float32), name='accuracy') 

        #labels = tf.to_int64(labels)
        ce_losses = tf.nn.sparse_softmax_cross_entropy_with_logits(logits, tf.cast(labels, tf.int32), name='ce')
        ce_loss = tf.reduce_mean(ce_losses, name='ce_loss')
        ...
        self.trn_acc = tf.placeholder(tf.float32)
        self.val_loss = tf.placeholder(tf.float32)
        self.val_acc = tf.placeholder(tf.float32)

        # summary
        tf.scalar_summary('lr', lr)
        tf.scalar_summary('loss', loss)
        tf.scalar_summary('trn_loss', self.trn_loss)
        tf.scalar_summary('trn_acc', self.trn_acc)
        tf.scalar_summary('val_loss', self.val_loss)
        tf.scalar_summary('val_acc', self.val_acc)
